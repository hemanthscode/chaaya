...
import { getTestimonials } from './testimonialService.js';
...
export const getHomepageData = async () => {
  const [
    ...
    Image.find({ featured: true, status: 'published' })
      .limit(8)
      .populate('category'),
    Image.find({ status: 'published' })
      ...
      .limit(12)
      .populate('category'),
    Series.find({ featured: true, status: 'published' })
      .populate('coverImage')
      .limit(6),
    Category.find().sort({ order: 1 }).limit(12),
    // use service-level function instead of Testimonial.getTestimonials(...)
    getTestimonials(4, true)
  ]);
...
export const getAboutData = async () => {
  const [totalImages, totalSeries, totalViews, topCategories] = await Promise.all([
...
export const generateSitemap = async () => {
  const [images, series, categories] = await Promise.all([
